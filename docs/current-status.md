...
- Root preparation/evidence suite: 24 tests passed. The full Miles suite remains at its previously documented baseline failures.
...
| GPU runtime and provenance | Strict dtype parity failed in job118; 30 A_log differences independently proved lossless BF16-to-FP32 widenings. | Version and test a narrow A_log widening rule without changing checkpoint weights or prior failures, then qualify EP8 trainer load/forward/backward. No optimizer execution yet. |
