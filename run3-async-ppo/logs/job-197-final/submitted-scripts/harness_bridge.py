...
async def generate(request: Request):
    ...
    data = response.json()
    meta = data["meta_info"]
    engine_version = meta.get("weight_version")
    if engine_version is not None and str(engine_version) != str(STATE.get("policy_version")):
        raise RuntimeError("Engine behavior policy changed within the rollout batch")
...
async def rollout(request: Request):
    ...
    async with LOCK:
        ...
        STATE["policy_version"] = int(body["weight_version"])
