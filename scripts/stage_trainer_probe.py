"""Stage committed trainer-probe code and submit a single telemetry-covered node."""
import argparse
import json
...
def main():
    ...
    if subprocess.check_output(['git', '-C', str(repo), 'status', '--porcelain'], text=True).strip():
        raise ValueError('Commit probe code and locks before submission.')
    revision = subprocess.check_output(['git', '-C', str(repo), 'rev-parse', 'HEAD'], text=True).strip()
    remote = '/shared/posttrainingx/runs/vultr-b200-slurm/' + run.root.name
    prefix = f'provenance/trainer-probe-code-v{args.attempt}/'
    phase = run.phase(f'02-trainer-probe-submission-v{args.attempt}')
    ...
    names = ['evidence.py', 'infra_node.py', 'enroot_run_config.py', 'fabric_probe.py', 'telemetry_native.py', 'telemetry_health.py',
             ...
             'checkpoint_parity.py', 'run_checkpoint_parity.py', 'trainer_probe.py', 'run_trainer_probe.py']
    ...
    command = ['python3', remote + '/' + prefix + 'run_trainer_probe.py', '--run-dir', remote,
    ...
    for payload in batches({'root': remote, 'create': False, 'manifest_sha256': sha256(run.root / 'run.json')}, files, limit=64*1024):
        ...
        if rc:
            phase.finish('fail', failure_summary='Immutable probe staging failed; reconcile before retrying.')
            return 1
    name = 'ptx-trainer-probe-lustre-' + run.root.name + '-v' + str(args.attempt)
    pod = pod_manifest(name, 'b200-nodepool-ac23753e6cfa', run.root.name)
    pod['metadata']['labels']['component'] = 'trainer-probe-lustre'
    pod['spec']['activeDeadlineSeconds'] = 1500
    container = pod['spec']['containers'][0]
    container['command'] = ['python3', '/run-artifacts/' + prefix + 'telemetry_lustre_host.py', '--run-dir', '/run-artifacts',
        '--hostname', 'gpu-nodes-0', '--duration-s', '1450', '--stream-label', f'lustre-trainer-probe-v{args.attempt}',
        '--job-marker', f'control/trainer-probe-job-v{args.attempt}.json',
        '--stop-marker', f'control/trainer-probe-lustre-v{args.attempt}.stop', '--role', 'trainer-probe']
    ...
    if rc:
        phase.finish('fail', failure_summary='Host collector not ready; no allocation submitted.')
        return 1
    rc, out, _ = phase.command(worker + ['sbatch', '--parsable', '--partition=gpu-nodes', '--nodes=1',
        ...
        '--time=00:25:00', '--no-requeue', '--job-name=ptx-trainer-probe-' + run.root.name + '-v' + str(args.attempt),
        '--chdir=' + remote, '--output=' + remote + f'/provenance/trainer-probe-v{args.attempt}-slurm-%j.out',
        '--error=' + remote + f'/provenance/trainer-probe-v{args.attempt}-slurm-%j.err', remote + '/' + prefix + 'submit.sbatch'])
    ...
    phase.finish('ok' if okay else 'fail', failure_summary=None if okay else 'Submission ambiguous; inspect unique job name before retry.',
        metadata={'slurm_job_id': job, 'source_git_sha': revision, 'collector_pod': name,
                  'scope': 'Submission receipt only; no trainer or quality success claimed.'})
