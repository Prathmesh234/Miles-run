...
from model_conversion import MILES_SHA, conversion_stem
...
def container_command(run, code, attempt, source, format_name='bf16'):
    stem = conversion_stem(format_name)
    root = run.root / f'images/{stem}-runtime-v{attempt}'
    root.mkdir(exist_ok=False)
    env = prepare(root)
    os.environ.update({key: value for key, value in env.items() if key.startswith('ENROOT_')})
    os.environ['NVIDIA_VISIBLE_DEVICES'] = 'all'
    nccl = run.root / f'telemetry/nccl/{stem}-v{attempt}'
    ...
    envs = {'NVIDIA_VISIBLE_DEVICES': 'all', 'PYTHONDONTWRITEBYTECODE': '1',
        ...
        'NCCL_DEBUG_FILE': f'/run-artifacts/telemetry/nccl/{stem}-v{attempt}/nccl.%h.%p.log'}
...
def main():
    ...
    phase = run.phase(f'02-{stem}-orchestration-v{args.attempt}')
    code = Path(__file__).resolve().parent
    errors, collector = [], None
    label = f'native-{stem}-v{args.attempt}'
    marker = run.root / f'control/{stem}-job-v{args.attempt}.json'
    atomic(marker, {'slurm_job_id': os.environ['SLURM_JOB_ID'], 'active': True})
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        try:
            if read_inventory(run, f'{stem}-start-v{args.attempt}'):
                raise RuntimeError('Fresh physical/Slurm inventory did not reconcile.')
            source = verify_sources(run)
            host = socket.gethostname()
            stats = run.root / f'telemetry/lustre-{stem}-v{args.attempt}' / host / 'lustre.jsonl.partial'
            if not stats.is_file():
                raise RuntimeError('Host Lustre collector is not ready; no conversion started.')
            collector = pool.submit(native_collector, run, code, args.attempt, args.format)
            ...
            atomic(run.root / f'control/{stem}-lustre-v{args.attempt}.stop', {'time': utcnow()})
            if collector is not None and collector.result():
                errors.append('Native collector exited with an error.')
            if read_inventory(run, f'{stem}-end-v{args.attempt}'):
    ...
    phase.finish('fail' if errors else 'ok', failure_summary='; '.join(errors) or None,
        metadata={'findings': errors, 'slurm_job_id': os.environ['SLURM_JOB_ID'],
                  'scope': 'One-node ' + args.format + '; not GRPO training or validated checkpoint parity.',
                  'artifacts': [f'tests/02-{stem}-child-v{args.attempt}', 'telemetry/' + label,
                                f'telemetry/lustre-{stem}-v{args.attempt}']}, refresh=False)
