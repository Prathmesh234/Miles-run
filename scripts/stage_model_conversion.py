...
from model_conversion import conversion_stem
...
def main():
    ...
    prefix = f'provenance/{stem}-code-v{args.attempt}/'
    phase = run.phase(f'02-{stem}-submission-v{args.attempt}')
    ...
    names = ['evidence.py', 'infra_node.py', 'enroot_run_config.py', 'fabric_probe.py', 'telemetry_native.py', 'telemetry_health.py',
             ...
             'convert_qwen_mxfp8.py', 'qwen_mxfp8_probe.py']
    ...
    command = ['python3', remote + '/' + prefix + 'run_model_conversion.py', '--run-dir', remote,
               '--attempt', str(args.attempt), '--format', args.format]
    ...
    phase.finish('ok' if okay else 'fail', failure_summary=None if okay else 'Submission ambiguous; inspect unique job name before retry.',
        metadata={'slurm_job_id': job, 'source_git_sha': revision, 'collector_pod': name, 'format': args.format,
